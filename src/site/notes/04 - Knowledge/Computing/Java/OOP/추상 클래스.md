---
{"dg-publish":true,"permalink":"/04 - Knowledge/Computing/Java/OOP/추상 클래스/","tags":["domain/java"]}
---
...
## 추상 메서드가 없는 추상 클래스
- Java에서는 `abstract` 키워드만 붙이면 추상 메서드가 없어도 추상 클래스로 선언 가능
- 직접 인스턴스화(`new`)는 불가능하지만, 서브클래스에 구현을 강제하지는 않음
- 주로 **"이 클래스는 직접 사용하지 말고 상속해서 써라"** 는 설계 의도를 표현할 때 사용
