---
{"dg-publish":true,"permalink":"/main/","metatags":{"google-site-verification":"20MA_khcMkhCRMF1pekcIxcPFGjEOr7Gyzxr3VbAaDo"},"tags":["gardenEntry"]}
---

## Yongho's Digital Garden

목표를 향해 흔들리지 않고, 차근차근 쌓아 튼튼한 뿌리를 만드는 백엔드 개발자입니다.
Java/Spring Boot · 모니터링 시스템 · AI Agent

---

## Computing

- [[02 - Knowledge/Computing/CS/_CS\|_CS]] — CS 기초 (OS, 네트워크, DB, 보안, 컴파일러)
- [[_Backend\|_Backend]] — 백엔드 설계 · 아키텍처
- [[02 - Knowledge/Computing/Web/_Web\|_Web]] — 웹 기술
- [[02 - Knowledge/Computing/Java/_Java\|_Java]] — Java
- [[02 - Knowledge/AI/_AI\|_AI]] — AI · LLM
- [[02 - Knowledge/Computing/Algorithm/_Algorithm\|_Algorithm]] — 알고리즘
- [[02 - Knowledge/Computing/DataStructure/_DataStructure\|_DataStructure]] — 자료구조
- [[02 - Knowledge/Computing/Git/_Git\|_Git]] — Git

## DevEnv

- [[02 - Knowledge/DevEnv/_DevEnv\|_DevEnv]] — 개발 환경 · 도구

## Projects

- [[03 - Projects/Home Server/Docker Compose 홈서버 구성\|Docker Compose 홈서버 구성]]
- [[03 - Projects/Home Server/GitHub Actions 배포 구축기\|GitHub Actions 배포 구축기]]
- [[03 - Projects/CBNU/Vagrant Ansible HTCondor 구축\|Vagrant Ansible HTCondor 구축]]
